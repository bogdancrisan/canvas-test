...
(function (Konva) {
  ...
  var ATTR_CHANGE_LIST = ['resizeEnabledChange', 'rotateAnchorOffsetChange', 'rotateEnabledChange', 'enabledAnchorsChange', 'anchorSizeChange', 'borderEnabledChange', 'borderStrokeChange', 'borderStrokeWidthChange', 'anchorStrokeChange', 'anchorStrokeWidthChange', 'anchorFillChange'].join(' ');

  var NODE_RECT = 'nodeRect';

  var TRANSFORM_CHANGE_STR = ['xChange.resizer', 'yChange.resizer', 'widthChange.resizer', 'heightChange.resizer', 'scaleXChange.resizer', 'scaleYChange.resizer', 'skewXChange.resizer', 'skewYChange.resizer', 'rotationChange.resizer', 'offsetXChange.resizer', 'offsetYChange.resizer', 'transformsEnabledChange.resizer'].join(' ');

  var REDRAW_CHANGE_STR = ['widthChange.resizer', 'heightChange.resizer', 'scaleXChange.resizer', 'scaleYChange.resizer', 'skewXChange.resizer', 'skewYChange.resizer', 'rotationChange.resizer', 'offsetXChange.resizer', 'offsetYChange.resizer'].join(' ');
  ...
  function getCursor(anchorName, rad, isMirrored) {
    ...
    if (Konva.Util._inRange(angle, 315 + 22.5, 360) || Konva.Util._inRange(angle, 0, 22.5)) {
      // TOP
      return 'ns-resize';
      ...
      // TODO: throw error
      Konva.Util.error('Transformer has unknown angle for cursor detection: ' + angle);
      return 'pointer';
    }
  }
  ...
  Konva.Transformer.prototype = {
    _centroid: false,
    ____init: function ____init(config) {
    ...
    _resetTransformCache: function _resetTransformCache() {
      this._clearCache(NODE_RECT);
      this._clearCache('transform');
      this._clearSelfAndDescendantCache('absoluteTransform');
    },

    _getNodeRect: function _getNodeRect() {
      return this._getCache(NODE_RECT, this.__getNodeRect);
    },

    __getNodeRect: function __getNodeRect() {
    ...
    _createAnchor: function _createAnchor(name) {
    ...
    _createBack: function _createBack() {
      var back = new Konva.Shape({
        ...
        sceneFunc: function sceneFunc(ctx) {
          ...
          if (tr.rotateEnabled()) {
            ctx.lineTo(this.width() / 2, -tr.rotateAnchorOffset() * Konva.Util._sign(this.height()));
          }
    ...
    _handleMouseMove: function _handleMouseMove(e) {
      ...
      if (this.movingResizer === 'top-left') {
        ...
        var newRotation = Konva.Util._radToDeg(rot) + Konva.Util._radToDeg(dAlpha);
        ...
        for (var i = 0; i < snaps.length; i++) {
          ...
          var dif = Math.abs(angle - Konva.Util._degToRad(newRotation)) % (Math.PI * 2);
        ...
        this._fitNodeInto({
          rotation: Konva.angleDeg ? newRotation : Konva.Util._degToRad(newRotation),
    ...
    _fitNodeInto: function _fitNodeInto(newAttrs, evt) {
    ...
    if (val instanceof Array) {
      val.forEach(function (name) {
        if (ANCHORS_NAMES.indexOf(name) === -1) {
          Konva.Util.warn('Unknown anchor name: ' + name + '. Available names are: ' + ANCHORS_NAMES.join(', '));
        }
      });
    }
    return val || [];
   ...
   */
  Konva.Factory.addGetterSetter(Konva.Transformer, 'enabledAnchors', ANCHORS_NAMES, validateResizers);
   ...
   */
  Konva.Factory.addGetterSetter(Konva.Transformer, 'anchorSize', 10, Konva.Validators.getNumberValidator());
   ...
   */
  Konva.Factory.addGetterSetter(Konva.Transformer, 'rotateAnchorOffset', 50, Konva.Validators.getNumberValidator());
   ...
   */
  Konva.Factory.addGetterSetter(Konva.Transformer, 'anchorStroke', 'rgb(0, 161, 255)');
   ...
   */
  Konva.Factory.addGetterSetter(Konva.Transformer, 'anchorStrokeWidth', 1, Konva.Validators.getNumberValidator());
   ...
   */
  Konva.Factory.addGetterSetter(Konva.Transformer, 'borderStroke', 'rgb(0, 161, 255)');
   ...
   */
  Konva.Factory.addGetterSetter(Konva.Transformer, 'borderStrokeWidth', 1, Konva.Validators.getNumberValidator());
   ...
   */
  Konva.Factory.addGetterSetter(Konva.Transformer, 'padding', 0, Konva.Validators.getNumberValidator());
